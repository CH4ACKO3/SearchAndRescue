...
public static class StandingTreatmentProbe {
 ...
 static void Setup(){
  ...
  if(scenario==0){CheckCoordinationOptions();CheckUninitializedWorkSettings();CheckScoreCache();CheckRetiredWorker();CheckSourceIndex();Rh2RoundProbe.Run(map,Check);SupplyEligibilityProbe.Run(doctor,patient,wound,Check);}
