...
& (Join-Path $PSScriptRoot 'SchedulerOptimizer/New-EngineProfile.ps1') -ModDir $root `
    -RuntimeDir $RuntimeDir -SaveData $SaveData -AdditionalMods $AdditionalMods | Out-Null
$package = 'ch4acko3.sarcaptureprobe.p' + [Guid]::NewGuid().ToString('N')
...
$arguments = @('-batchmode','-nographics','-quicktest','-sar-bench-worker','-sar-capture-probe',
