...
**Ancient Urban Ruins — 兼容**
支持 SAR 治疗使用多次使用医疗包，包括未开封和最后一次使用的情况。这类医疗包使用普通治疗而非 CE 稳定或 RH2 急救，以保留原生次数消耗逻辑。此适配范围为医疗物资，不代表覆盖 AUR 的所有功能。
...
识别原生无脉搏、缺氧、肝/肾/肠道衰竭及昏迷，提高撤离到可用床位的优先级；无脉搏和缺氧权重更高。不可包扎状态不产生无效治疗，功能恢复后撤销额外权重。手术与生命维持仍由对应系统处理，不自动添加手术账单。
...
**Dubs Rimkit — 联动**
佩戴有剩余次数的医疗包并开启其原生自动治疗开关后，SAR 可主动调用 BandageOthers。遵守医生权限和用药策略，到达时重新检查医疗包，每轮原生包扎消耗一份后重新评估，非紧急伤口留到安全位置。可执行 CE 稳定时保留 CE 流程。医疗包补充和手动包扎沿用原模组。
...
SAR 没有可执行救援分配时，盟友可接手已标记患者。已分配治疗或搬运继续保护相应阶段，仍敌对目标的俘虏顺序及玩家强制命令保持原样。

**No One Left Behind — 部分兼容**
敌军搬运者在实际携带伤员期间持有运输所有权，撤退救援由 No One Left Behind 执行。敌对搬运结束后，仍然有效的 SAR 标记会重新进入调度。

**MedPod — 兼容**
搬入、救援及自行入舱任务纳入所有权，实际占用医疗舱后交给设施处理。SAR 无可执行分配时，标记不会阻止原生入舱；已分配救援保留当前阶段。选舱和舱内治疗仍由原模组负责。
...
**GrimWorks: Work Manager — 联动**
遵守细分工作优先级与禁用设置。同时开启战地救援与原生 Rescue 工作即可搬运伤员；急救仍需医疗工作授权。GrimWorks 的 Nurse 工作不会赋予治疗权限。与 Work Tab 同时使用时，任务需在两个管理器中均被允许。

**Work Tab — 兼容**
读取细分 WorkGiver 优先级。“执行紧急医疗”配合战地救援可用于急救专职人员。与 More Injuries 并用时，CPR、气道管理、除颤及医生输液输血还需开启对应原生子工作；护理保持独立授权。常规治疗与手术可关闭，常规治疗区分人形和动物子工作。Work Tab 不是必需依赖。
