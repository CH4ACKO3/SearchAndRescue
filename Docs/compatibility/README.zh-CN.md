...
支持 SAR 治疗使用多次使用医疗包，包括未开封和最后一次使用的情况。SAR 通过普通治疗使用这类医疗包，保留原生使用次数消耗逻辑。此适配支持 AUR 的医疗物资。
...
遵守细分工作优先级与禁用设置。同时开启战地救援与原生 Rescue 工作即可搬运伤员；急救仍需医疗工作授权。包扎需开启对应医疗工作；GrimWorks 的 Nurse 工作负责护理。与 Work Tab 同时使用时，任务需在两个管理器中均被允许。

**Work Tab — 兼容**
读取细分 WorkGiver 优先级。“执行紧急医疗”配合战地救援可用于急救专职人员。与 More Injuries 并用时，CPR、气道管理、除颤及医生输液输血还需开启对应原生子工作；护理保持独立授权。常规治疗与手术可关闭，常规治疗区分人形和动物子工作。Work Tab 为可选模组。
