...
Recognizes native no-pulse, oxygen-deprivation, liver/kidney/intestinal failure and coma states, adding priority to evacuation to an available bed. No-pulse and oxygen-deprivation states receive more weight. Non-tendable conditions do not create futile treatment jobs, and restored function clears the extra weight. Surgery and life support remain native; SAR does not add surgery bills.
...
**Dubs Rimkit — Integration**
SAR can dispatch native BandageOthers with a worn, charged kit whose native automatic-tending toggle is enabled. Doctor permissions and medicine policy apply. The kit is rechecked on arrival; each native round consumes one charge and ends for reassessment, deferring minor field wounds until a safe care location. Eligible CE stabilization keeps its own flow. Reloading and manual bandaging remain native.
...
Allies can take over marked patients when SAR has no executable assignment. Committed treatment or transport retains its phase; hostile capture ordering and player-forced commands remain protected.
...
**MedPod — Compatible**
Pod delivery, rescue and self-entry participate in ownership; actual pod occupancy hands care to the facility. Unassigned SAR marks allow native admission, while committed SAR care retains its phase. Pod selection and treatment remain native.

**RH2 — BCD: CASEVAC — Integration**
Workers assigned to Field Rescue and CASEVAC start unattended evacuations first, then join existing teams or upgrade nearby SAR transports. Manual commands retain control. Upgrades require a nearby carrier and enough remaining route; rescue points retain ordinary evacuation.
...
**Hauler's Dream — Integration**
Keeps medicine reserved for active SAR treatment and supply tasks from being unloaded as surplus. Normal unloading resumes when SAR releases the supplies.
